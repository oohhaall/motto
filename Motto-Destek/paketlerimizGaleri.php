...
if(b($_GET['sil'])){
	...
	if($_POST['silOnay'] == "onaylandi"){
		$silSra = $DB->qry("SELECT sra, resim FROM paketlerimizgaleri WHERE id=".STR($_GET['sil'], "int"),1);
		$silSraDuzenle = $DB->qry("UPDATE paketlerimizgaleri SET sra=sra-1 WHERE uid=".$kid." AND sra>".STR($silSra[0]['sra'], "int"),0);
		$DB->dosyaDel(array('../resim/'.$silSra[0]['resim']));
		$sil = $DB->qry("DELETE FROM paketlerimizgaleri WHERE id=".STR($_GET['sil'], "int"),0);
...
if(b($_GET['y'])){
	$ssra = $DB->qry("SELECT sra FROM paketlerimizgaleri WHERE uid=".$kid." AND id=".STR($_GET['y'], "int"),1);
	$yukari = $DB->qry("UPDATE paketlerimizgaleri SET sra=sra+1 WHERE uid=".$kid." AND sra > 0 AND sra=".STR(($ssra[0]['sra']-1), "int"),0);
	$yukari2 = $DB->qry("UPDATE paketlerimizgaleri SET sra=sra-1 WHERE uid=".$kid." AND sra > 1 AND id=".STR($_GET['y'], "int"),0);
	header('Location: '.$self.'&s='.$_GET['y']);	exit;
}
if(b($_GET['a'])){
	$ssra = $DB->qry("SELECT sra FROM paketlerimizgaleri WHERE uid=".$kid." AND id=".STR($_GET['a'], "int"),1);
	$ssys = $DB->qry("SELECT sra FROM paketlerimizgaleri WHERE uid=".$kid,3);
	$asagi = $DB->qry("UPDATE paketlerimizgaleri SET sra=sra-1 WHERE uid=".$kid." AND sra < ".($ssys+1)." AND sra=".STR(($ssra[0]['sra']+1), "int"),0);
	$asagi2 = $DB->qry("UPDATE paketlerimizgaleri SET sra=sra+1 WHERE uid=".$kid." AND sra < ".$ssys." AND id=".STR($_GET['a'], "int"),0);
...
$veriler = $DB->qry("SELECT * FROM paketlerimizgaleri WHERE uid=".$DB->str($_GET['k'],'int')." ORDER BY sra ASC",1);
$sira = 0;
