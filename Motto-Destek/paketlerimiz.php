...
if(b($_GET['sil'])){
	...
	if($_POST['silOnay'] == "onaylandi"){
		...
		$urunslayt = $DB->qry("SELECT resim FROM paketlerimizgaleri WHERE uid=".STR($_GET['sil'], "int"),5);
		if(count($urunslayt)>0){
		$DB->dosyaDel($urunslayt,'../resim/');
		$DB->qry("DELETE FROM paketlerimizgaleri WHERE uid=".STR($_GET['sil'], "int"),0);
		}


		$DB->qry("DELETE FROM paketlerimiz WHERE id=".STR($_GET['sil'], "int"),0);
...
$veriler = $DB->qry("SELECT *, DATE_FORMAT(tarih, '%d.%m.%Y') as trh, DATE_FORMAT(tarih, '%d %M %Y') as trhAy FROM paketlerimiz ORDER BY tarih DESC",1);
$sira = 0;
foreach($veriler as $k){
	...
	$kStr .= '<ul'.$stl.'>
				...
				<a href="paketlerimizDuzenle.php?duzenle='.$k['id'].'" title="Düzenle"><li class="btn"><img src="images/edit.png" width="24" height="24" border="0" /></li></a>
				<a class="frmPop" href="paketlerimizGaleri.php?k='.$k['id'].'"><li class="btn"><img title="Fotoğraf Ekle" src="images/img.png" width="24" height="24" border="0" /></li></a>
  ...
  <tr>
    <td height="0" align="left" valign="top">
	 <div class="banAltMenu">
	  	<a href="<?php echo $tablo; ?>.php"><li id="Clk">Paketlerimiz Menüler</li></a>
        <a href="<?php echo $tablo; ?>paketlerimizEkle.php"><li>Paketlerimiz Ekle</li></a> 
